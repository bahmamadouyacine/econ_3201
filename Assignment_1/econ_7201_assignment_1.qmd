...
(c) Generate two variables $z_1$ and $z_2$ that take on the following values: $$ 𝑧1 = √−2ln(𝑢1) ×cos(2𝜋𝑢2)$$ and $$z_1=\sqrt{-2\ln(u_1)}\times\text{sin}(2\pi u_2).$$ In R, $\sqrt{\text{  }}$ is computed using `sqrt()`, $\ln$ is computed using `log()`, $\text{cos}$ is computed using `cos()`, and $\text{sin}$ is computed using `sin()`. 
